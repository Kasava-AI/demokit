...
export default function Checkout() {
  ...
  const { cart, products, isLoading: isCartLoading, mutate } = useCart()
  const { checkout, isSubmitting, result } = useCheckout()
  ...
  useEffect(() => {
    if (result?.success && result.orderId) {
      mutate() // Refresh cart (should be empty now)
      navigate(`/order-confirmation/${result.orderId}`)
    }
  }, [result, navigate, mutate])

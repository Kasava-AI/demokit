...
import type { OrdersLoaderData, Product } from '@/types'
...
export default function Orders() {
  const { orders } = useLoaderData() as OrdersLoaderData
  ...
  if (orders.length === 0) {
    return (
      <div className={`container mx-auto px-4 py-16 transition-opacity ${isLoading ? 'opacity-50' : ''}`}>
  ...
  return (
    <div className={`container mx-auto px-4 py-8 transition-opacity ${isLoading ? 'opacity-50' : ''}`}>
          ...
          <div key={order.id} className="card overflow-hidden">
            ...
            <div className="divide-y p-4">
              {order.items.map((item) => {
                const product = demoProducts.find((p: Product) => p.id === item.product_id)
